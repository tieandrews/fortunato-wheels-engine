...
def calculate_evaluation_metrics(y, y_pred, metrics=["rmse", "mape", "r2"]):
    ...
    if "rmse" in metrics:
        evaluation_metrics["rmse"] = round(mean_squared_error(y, y_pred, squared=False), 1)
    ...
    metrics_df = pd.DataFrame.from_dict(evaluation_metrics, orient="index").reset_index().rename(columns={"index": "metric", 0: "value"})
...
def calculate_evaluation_metrics_by_model(df, metrics=["rmse", "mape", "r2"]):
    ...
    if not all([col in df.columns for col in ['make', 'model', 'price', 'predicted_price']]):
        raise ValueError("df must contain make, model, price, and predicted_price columns")
    ...
    for make, model in unique_make_models.values:
        ...
        if "rmse" in metrics:
            rmse = mean_squared_error(y_actual_subset, y_predicted_subset, squared=False)
            make_model_results[(make, model)]["RMSE"] = round(rmse, 1)

        if "mape" in metrics:
            mape = np.mean(np.abs((y_actual_subset - y_predicted_subset) / y_actual_subset)) 
